...
# match.  (The ones that match might be different each time the rule
# is encountered.)  For example, the rule 2: 1 3 | 3 1 means that to
# match rule 2, the text being checked must match rule 1 followed by
...
# --------------------
#
# We convert the rules to regular expressions.

open("19.in") do |f|
  p1, p2 = f.read.split("\n\n")
  Rules = p1.split("\n").map {|l|
    if !(m = l.match(/^(\d+): "([a-z]+)"$/)).nil?
      [m[1], m[2]]
    elsif !(m = l.match(/^(\d+): (\d+(?: \d+)*(?: \| \d+(?: \d+)*)*)$/)).nil?
      [m[1], m[2].split("|").map(&:split)]
    end
  }.to_h
  Messages = p2.split("\n")
end
...
def expand(id)
  r = Rules[id]
  if r.class == Array
    r.map {|a| a.map {|r| "(?:#{expand(r)})" }.join }.join("|")
  else
    r
  end
end

rule = Regexp.new("^(?:#{expand("0")})$")
puts Messages.count {|m| rule.match(m) }
...
# --------------------
#
# The key (modified) rules are these:
#
# 0: 8 11
# 8: 42 | 42 8
# 11: 42 31 | 42 11 31
#
# Rules 0, 8, 11, 31, and 42 are referenced nowhere else.  Thus, the
# overall pattern is m rule 42s followed by n rule 31s with m > n > 0.
# This takes us outside the domain of regular expressions.  We could
# use Ruby's extended \g recursive subexpression operator, but instead
# continue to use ordinary regular expressions for matching rules 42
# and 31, and manually govern the process of counting those matches.
#
# Note that this technique is not guaranteed to work in general.
# Rules 42 and 31 contain alternatives.  Within a regular expression
# the engine will backtrack and try alternatives to achieve a global
# match, but outside the engine we will receive only the first match
# with no means to revisit.  For example, consider:
#
# 42: aa | a
# 31: b
#
# Given message "aab", 42 will match against "aa" first, yielding an
# overall mismatch (because 31 will match "b" and then the number of
# 42s does not exceed the number of 31s), when instead matching 42
# twice using the "a" alternative would yield a match.  Fortunately,
# in our case all alternatives of each rule have the same length (we
# verified this).  Thus in a sense the alternatives collapse, and a
# rule either matches or it doesn't.

Rule42 = Regexp.new("^(?:#{expand("42")})")
Rule31 = Regexp.new("^(?:#{expand("31")})")

def match(s, seen31=false)
  # Returns a list of 2-tuples, each of which represents a match and
  # gives the number of 42s that matched followed by the number of
  # 31s.
  return [[0, 0]] if s.length == 0
  r = []
  if !seen31
    if !(m = Rule42.match(s)).nil?
      r.concat(match(s[m.end(0)..-1]).map {|n42,n31| [n42+1, n31] })
    end
  end
  if !(m = Rule31.match(s)).nil?
    r.concat(match(s[m.end(0)..-1], true).map {|n42,n31| [n42, n31+1] })
  end
  r
end

puts Messages.count {|m| match(m).any? {|n42,n31| n42 > n31 && n31 > 0 } }
