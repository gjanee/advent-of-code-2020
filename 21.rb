...
# --------------------
#
# This puzzle is an almost exact repeat of day 16.

require "set"

Food = Struct.new(:ingredients, :allergens)

foods = open("21.in").map {|l|
  m = l.match(/^(.*) \(contains (.*)\)$/)
  Food.new(m[1].split, m[2].split(", "))
}

# assemble table of allergens, to be filled in later
# mapping: { allergen => ingredient }
mapping = foods.map {|f| f.allergens }.flatten.uniq.map {|a| [a, nil] }.to_h

# sets: { allergen => list of possible ingredients }
sets = mapping.keys.map {|a|
  [ a,
    foods
      .select {|f| f.allergens.member?(a) }
      .map {|f| Set.new(f.ingredients) }
      .reduce(:intersection)
  ]
}.to_h
...
while sets.length > 0
  a = sets.find {|a,l| l.length == 1 }.first
  i = sets[a].first
  mapping[a] = i
  sets.delete(a)
...
puts foods.map {|f|
  f.ingredients.count {|i| !mapping.values.member?(i) }
}.reduce(:+)
...
puts mapping.sort_by(&:first).map(&:last).join(",")
