...
Initial_decks = open("22.in")
  .read
  .split("\n\n")
  .map {|p| p.split("\n")[1..-1].map(&:to_i) }

def play(decks)
  # plays a game, modifying `decks` and returning the index of the winner
  win = nil
  while decks.all? {|d| d.length > 0 }
    win = decks.map(&:first).each_with_index.max.last
    lose = 1-win
    decks[win] << decks[win].shift << decks[lose].shift
  end
  win
end

def score(deck)
  deck.reverse.each_with_index.map {|c,i| c*(i+1) }.reduce(:+)
end

decks = Initial_decks.map(&:clone)
puts score(decks[play(decks)])

# --- Part Two ---
#
# You lost to the small crab!  Fortunately, crabs aren't very good at
# recursion.  To defend your honor as a Raft Captain, you challenge
...
# only fair).  Then, the game consists of a series of rounds with a
# few changes:
#
...
# deck (again so that the winner's card is above the other card).
# Note that the winner's card might be the lower-valued of the two
# cards if they won the round due to winning a sub-game.  If
# collecting cards by winning the round causes a player to have all of
...
# trigger the sub-game).  During this sub-game, the game that
# triggered it is on hold and completely unaffected; no cards are
# removed from players' decks to form the sub-game.  (For example, if
# player 1 drew the 3 card, their deck in the sub-game would be copies
# of the next three cards in their deck.)
#
...
# game of Recursive Combat using the same two decks as before.  What
# is the winning player's score?
#
# --------------------
#
# Ruby performance is frustrating at times.  One of the joys of Ruby
# is writing in a functional style, but the cost of creating lots of
# intermediate objects on the heap can be prohibitive.  For example,
# m = [a, b].max is almost 6x slower than m = a > b ? a : b.  In this
# puzzle, the use of a set below to store seen configurations is
# extremely sensitive to the representation of the configurations:
#
# time (s) | representation
# ---------+-------------------------------------------
#     13.3 | decks.map(&:clone) (i.e., array of arrays)
#      8.6 | decks.to_s
#      7.0 | decks.hash
#      5.1 | (decks[0]+[0]+decks[1]).pack("c*")
...
def play(decks)
  ...
  win = nil
  while decks.all? {|d| d.length > 0 }
    h = (decks[0]+[0]+decks[1]).pack("c*")
    return 0 if seen.member?(h)
    seen << h
    if decks.all? {|d| d.length > d[0] }
      win = play(decks.map {|d| d.slice(1, d[0]) })
    else
      win = decks.map(&:first).each_with_index.max.last
    end
    lose = 1-win
