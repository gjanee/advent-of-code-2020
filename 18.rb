...
# operator ::= "*" | "+"
#
# Our parse functions each accept a token list and starting index,
# process (some of) the token list, and return an evaluation value and
# continuation index.

def term(tl, i)
  if tl[i] == "("
    n, i = expression(tl, i+1)
    [n, i+1]
  else
    [tl[i].to_i, i+1]
  end
