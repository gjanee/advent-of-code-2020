...
# --------------------
#
# Since the number of active cubes relative to the diameter of the
# space is small, we represent the grid as a set of coordinates of
# active cubes (only).  To compute the next cycle we visit the
# neighbors of each active cube, counting how many active cubes each
# neighbor is adjacent to.  To be active in the next cycle a cube must
# be adjacent to multiple currently active cubes, hence we are
# justified in visiting only neighbors.
...
def neighbors(coord)
  # returns coordinates of the 3^D-1 neighbors of a coordinate tuple
  delta = [-1, 0, 1]
  delta
    .product(*([delta]*($D-1)))
    .reject {|t| t == [0]*$D }
    .map {|t|
      t.zip(coord).map {|a,b| a+b }
    }
end

def load
  active = Set.new
  open("17.in").each_with_index.map do |l,x|
    l.chomp.chars.each_with_index do |cube,y|
      active << [x,y] + [0]*($D-2) if cube == "#"
    end
  end
  active
end
...
def cycle(active)
  ...
  counts.each do |coord,v|
    if active.member?(coord)
      new_active << coord if v == 2 || v == 3
    else
      new_active << coord if v == 3
    end
  end
  new_active
end
