...
class Bag
  ...
  def Bag.[](color)
    @@all.member?(color) ? @@all[color] : Bag.new(color)
  end

  attr_reader :color, :inside

  def initialize(color)
    @color = color
    @contents = [] # [[count, contained bag], ...]
    @inside = []   # [containing bag, ...]
    @@all[color] = self
  end

  def contains!(n, other)
    @contents << [n, other]
    other.inside << self
  end

  def containing_colors
    s = Set.new(@inside.map(&:color))
    @inside.map(&:containing_colors).reduce(s, :union)
  end
...
open("07.in").each do |l|
  ...
  b = Bag[m[1]]
  m[2].split(",").reject {|c| c == " no other bags" }.each do |c|
    m = c.match(/^ (\d+) (.*) bags?$/)
    b.contains!(m[1].to_i, Bag[m[2]])
  end
end

puts Bag["shiny gold"].containing_colors.length
...
class Bag
  def contained_count
    @contents.map {|n,b| n*(b.contained_count+1) }.reduce(0, :+)
  end
end

puts Bag["shiny gold"].contained_count
