...
# --------------------
#
# We rely on the statement that numbers within a window are unique.
# (Numbers are not unique across the entire puzzle input, but they are
# unique within any given window.)

require "set"

W = 25 # window size
Numbers = open("09.in").read.split.map(&:to_i)

pool = Hash.new {|h,k| h[k] = Set.new } # sum => addends

# load preamble
Numbers[0,W].combination(2).each do |a,b|
  pool[a+b] << a << b
end

outlier = nil
(W...Numbers.length).each do |i|
  n = Numbers[i]
  if !pool.member?(n)
    outlier = n
    break
  end
  # remove oldest number from pool
  o = Numbers[i-W] # outgoing number
  pool.keys.each do |s|
    if pool[s].member?(o)
      pool[s].delete(o)
      pool[s].delete(s-o)
      pool.delete(s) if pool[s].length == 0
    end
  end
  # add new number to pool
  Numbers[i-W+1...i].each do |m|
    pool[n+m] << n << m
  end
end
...
Numbers.each_index do |i|
  ...
  (i+1...Numbers.length).each do |j|
    ...
    if s >= outlier
      if s == outlier
        puts Numbers[i..j].minmax.reduce(:+)
        done = true
      end
      break
    end
