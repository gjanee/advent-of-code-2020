...
# significant bit (2^0, that is, the 1s bit) on the right.  The
...
def solve(update_mask, write_value)
  mem = {}
  open("14.in").each do |l|
    if !(m = l.match(/^mask = ([01X]{36})$/)).nil?
      update_mask.call(m[1])
    elsif !(m = l.match(/^mem\[(\d+)\] = (\d+)$/)).nil?
      write_value.call(mem, m[1].to_i, m[2].to_i)
    end
  end
  mem.values.reduce(:+)
end

pmask = nil # positive (bit-setting) mask
nmask = nil # negative (bit-clearing) mask

update_mask = lambda {|mask|
  pmask = mask.tr("X", "0").to_i(2)
  nmask = mask.tr("X", "1").to_i(2)
}

write_value = lambda {|mem, address, value|
  mem[address] = (value|pmask)&nmask
}

puts solve(update_mask, write_value)
...
pmask = nil     # positive (bit-setting) mask
x_indexes = nil # indexes of X's in mask

update_mask = lambda {|mask|
  pmask = mask.tr("X", "0").to_i(2)
  x_indexes = mask.chars.each_with_index.select {|d,i| d == "X" }.map(&:last)
}

write_value = lambda {|mem, address, value|
  (2**x_indexes.length).times do |i|
    a = "%036b" % (address|pmask)
    ("%0*b" % [x_indexes.length, i]).chars.each_with_index do |d,i|
      a[x_indexes[i]] = d
    end
    mem[a.to_i(2)] = value
  end
