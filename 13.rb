...
# bus.  The second line lists the bus IDs that are in service
# according to the shuttle company; entries that show x must be out of
...
open("13.in") do |f|
  Timestamp = f.readline.to_i
  Buses = f.readline.split(",").map {|b| b == "x" ? nil : b.to_i }
end

wait, bus = Buses.reject(&:nil?).map {|b|
  m = Timestamp%b
  [m == 0 ? m : b-m, b]
}.min
puts wait*bus
...
# --------------------
#
# Very similar puzzle to year 2016, problem 15.  Since the bus IDs are
# coprime (all prime even), a straightforward application of the
...
modulus = Buses.reject(&:nil?).reduce(:*)
puts Buses.each_with_index.reject {|b,i| b.nil? }.map {|b,i|
  mm = modulus/b
  -i*mm*inverse(mm, b)
}.reduce(:+) % modulus
