...
#   the current cup's label minus one.  If this would select one of
...
# --------------------
#
# As will become more apparent in part 2, we can assume that cups are
# labeled sequentially from 1 to the number of cups.  We represent the
# cups as a circular, singly linked list, but store the list in an
# array indexed by cup number (entry 0 is unused).  Specifically, a[i]
# is the number (=index) of the cup that follows cup i.

Input = "487912365".chars.map(&:to_i)

# The following function is better expressed as a lambda inside `move`
# below, but doing so doubles the execution time.
def decr(c, n)
  c > 1 ? c-1 : n-1
end

def move(a, cc)
  # Performs a move, starting from current cup `cc` and modifying
  # array/linked list `a`.  Returns the new current cup.
  removed = [a[cc], a[a[cc]], a[a[a[cc]]]]
  a[cc] = a[removed[2]]
  dc = decr(cc, a.length) # destination cup
  while removed.member?(dc)
    dc = decr(dc, a.length)
  end
  a[removed[2]] = a[dc]
  a[dc] = removed[0]
...
Input.each_index do |i|
  a[Input[i]] = Input[(i+1)%Input.length]
end
...
# proceeding one by one until one million is reached.  (For example,
# if your labeling were 54321, the cups would be numbered 5, 4, 3, 2,
# 1, and then start counting up from 6 until one million is reached.)
# In this way, every number from one through one million is used
# exactly once.
#
...
# setup
a = (0..1000000).map {|c| c+1 }
(0..Input.length-2).each do |i|
  a[Input[i]] = Input[i+1]
end
a[Input[-1]] = Input.length+1
a[-1] = Input[0]
