...
def run
  # returns [completed normally?, accumulator]
  ip = 0
  acc = 0
  visited = Array.new(Program.length, false)
  while ip < Program.length
    return [false, acc] if visited[ip]
    visited[ip] = true
    case Program[ip][0]
    when "acc"
      acc += Program[ip][1]
...
# --------------------
#
# We're tempted to find a more analytic approach, but nothing simple
# comes to mind and the small size of the problem doesn't demand one.

Program.each_index.reject {|i| Program[i][0] == "acc" }.each do |i|
  toggle = lambda {|i|
