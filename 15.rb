...
#   spoken before then (turn 1).  Thus, the 5th number spoken is
#   4 - 1, 3.
...
# - Given the starting numbers 3,1,2, the 2020th number spoken is
#   1836.
#
...
# --------------------
#
# This is Van Eck's sequence, OEIS A181391.  Only recently cataloged,
# little is known about it.  Note that we don't need to retain the
# entire sequence, only the most recent positions for each value
# encountered so far, which we do using a dictionary.

Input = [1, 0, 15, 2, 10, 13]

def solve(turns)
  seen = Input[0..-2].each_with_index.map {|n,i| [n, i] }.to_h
  last = Input[-1] # last number spoken
  i = Input.length-1
  (turns-Input.length).times do
    if seen.member?(last)
