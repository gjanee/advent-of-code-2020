...
# train.  However, the train ticket you were given is in a language
# you don't understand.  You should probably figure out what it says
...
open("16.in") do |f|
  p1, p2, p3 = f.read.split("\n\n")
  Rules = p1.split("\n").map {|l|
    m = l.match(/^(.*): (\d+)-(\d+) or (\d+)-(\d+)$/)
    [m[1], [Range.new(m[2].to_i, m[3].to_i), Range.new(m[4].to_i, m[5].to_i)]]
  }.to_h
  My_ticket = p2.split("\n")[1].split(",").map(&:to_i)
  Nearby_tickets = p3.split("\n")[1..-1].map {|l| l.split(",").map(&:to_i) }
end

def invalid?(v)
  !Rules.values.flatten.any? {|r| r.cover?(v) }
end
...
# --------------------
#
# Similar to year 2018, day 16.  The puzzle supports simple, direct
# deduction: at each step we can unambiguously identify at least one
# field.

require "set"

# identify possible fields for each column of values
# sets: { column index => set of field names }
sets = My_ticket.each_index.map {|i|
  [ i,
    Nearby_tickets
      .reject {|t| t.any? {|v| invalid?(v) } }
      .map {|t| t[i] }
      .map {|v|
        Set.new(
          Rules
            .select {|f,l| l.any? {|r| r.cover?(v) } }
            .map(&:first)
        )
      }
      .reduce(:intersection)
...
# deduce mappings one by one
# mapping: [field name, ...]
mapping = []
while sets.length > 0
  i = sets.find {|i,s| s.length == 1 }.first
  f = sets[i].first
  mapping[i] = f
  sets.delete(i)
...
puts My_ticket
  .each_with_index
  .select {|v,i| mapping[i].start_with?("departure") }
  .map(&:first)
  .reduce(:*)
